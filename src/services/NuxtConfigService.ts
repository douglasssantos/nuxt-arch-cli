...
export class NuxtConfigService {
   ...
   */
  async addLayer(nuxtConfigPath: string, layerPath: string): Promise<boolean> {
    ...
    let extendsNode = null as import('ts-morph').ArrayLiteralExpression | null
    ...
    for (const call of callExpressions) {
      ...
      if (expr.getText() !== 'defineNuxtConfig') continue
    ...
    if (!extendsNode) {
      throw new Error(
        'Could not find defineNuxtConfig or extends array in nuxt.config.ts. ' +
   ...
   */
  async detectNuxtVersion(cwd: string): Promise<'v3' | 'v4'> {
    try {
