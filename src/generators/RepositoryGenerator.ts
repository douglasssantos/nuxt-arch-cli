...
export class RepositoryGenerator extends BaseGenerator {
  async generate(options: GeneratorOptions): Promise<void> {
    ...
    // Domain interface
    const interfacePath = this.pathResolver.layerPath(
      options.layer,
      'domain/repositories',
      `I${names.pascal}Repository.ts`,
    )
    ...
    const implPath = this.pathResolver.layerPath(
      options.layer,
      'infrastructure/repositories',
      `${names.pascal}Repository.ts`,
    )
    await this.generateFile(
      implPath,
      'repository-impl',
    ...
    // Update barrel files
    const domainBarrel = this.pathResolver.layerPath(
      options.layer,
      'domain/repositories',
      'index.ts',
    )
    const infraBarrel = this.pathResolver.layerPath(
      options.layer,
      'infrastructure/repositories',
      'index.ts',
    )
